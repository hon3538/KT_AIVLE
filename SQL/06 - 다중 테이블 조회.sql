...
SELECT v.emp_id, e.emp_name, v.begin_date, v.reason, v.duration
	FROM vacation AS v
...
SELECT dept_id, dept_name, u.unit_name
	FROM department AS d
    JOIN unit AS u ON d.unit_id = u.unit_id;
-- 부서가 7개였는데 6개가 됨..unit id가 없는 부서 있기 때문이다 -> outer join필요
...
SELECT emp_id, emp_name, dept_id, phone, email
	...
	AND EXISTS (SELECT * 
                    FROM vacation
                    WHERE emp_id = e.emp_id);   # inner join과 같은 문법 
