...
SELECT 'Hello SQL World' from dual;  # oracle 문법
...
SELECT emp_id, emp_name, hire_date, gender FROM employee WHERE gender = 'M';


-- Q) 2020년 6월 1일 입사자의 이름, 부서코드, 성별, 입사일, 이메일 정보 조회
SELECT emp_name, dept_id, gender, hire_date, email FROM employee WHERE hire_date = '2020-06-01';
...
SELECT emp_id, begin_date, reason, duration FROM vacation WHERE duration >= 5;
...
SELECT * FROM employee WHERE not phone LIKE '010%';
...
SELECT emp_name, emp_id, dept_id, hire_date, phone
	FROM employee
	WHERE YEAR(hire_date) = 2020; # 성능이슈로 where 절에는 함수 사용 지양
...
SELECT * FROM employee WHERE hire_date >= '2022-01-01' AND dept_id = 'SYS';
